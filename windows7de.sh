...
while getopts 'sfdah' OPTION; do
	case $OPTION in
		s)
			...
			git clone https://www.github.com/aeroshell-desktop/aerothemeplasma;
			cd aerothemeplasma;
		...
		d)
			...
			;;
		a)
			echo "Starting installation of Windows 7 on linux kernel.."
			cd;
			...
			cd yay;
			makepkg -si;
			yay -S aerothemeplasma-desktop-x11 uac-polkit-agent aerothemeplasma-desktop aeroshell-libplasma aeroshell-workspace aeroshell-kwin-components aeroshell-smod aeroshell-smodglow-x11 aerothemeplasma-sounds aerothemeplasma-icons ksysguard6-git;
			;;
		h)
			echo "  			-a stands for arch
			  ...
			  This is a script for installing Windows 7 on linux kernel."
			;;
	esac
done
